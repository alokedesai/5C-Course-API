...
for i in range(0, len(courses)):
  course = {
          "name" : nulltest(names[i]),
          'major': nulltest(str(majors[i]).strip()),
          'number' : nulltest(int(number[i])),            
          "teacher" : nulltest(profs[i][0:(profs[i].find("credit")-3)]),
          "course" : majors[i] + number[i],
          "school" : nulltest(schools[i]),
          'description': nulltest(description[i])}
